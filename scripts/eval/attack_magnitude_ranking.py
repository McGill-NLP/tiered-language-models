...
Reports accuracy / precision / recall / F1 / ROC-AUC for the methods:
  ...
  mlp_outlier    — rank MLP columns by |z-score|, predict top-K%
  attn_outlier   — rank attn heads by |z-score|, predict top-K%
  combined_smallest — one pool: smallest MLP + smallest attn
  combined_outlier  — one pool: largest |z| MLP + largest |z| attn
"""
...
def _evaluate(z: np.ndarray, labels: np.ndarray, direction: str) -> dict | None:
    ...
    if direction == "smallest":
        ...
        pred_idx = np.argsort(-z)[:k_true]
        auc_scores = z
    elif direction == "outlier":
        abs_z = np.abs(z)
        pred_idx = np.argsort(-abs_z)[:k_true]
        auc_scores = abs_z
    else:
...
def main() -> None:
    ...
    methods["mlp_outlier"] = _evaluate(mlp_z, mlp_labels, "outlier")
    methods["attn_outlier"] = _evaluate(attn_z, attn_labels, "outlier")
    ...
    methods["combined_outlier"] = _evaluate(combined_score, combined_labels, "outlier")
